...
```js
let reg = /china/;
console.log(reg.exec("hello, china"));
...
```js
let reg = /向./;
console.log(reg.test("向前"));	// true
console.log(reg.test("向后"));	// true
console.log(reg.test("向上"));	// true
console.log(reg.test("向下"));	// true
console.log(reg.test("向左"));	// true
console.log(reg.test("向右"));	// true

console.log(reg.test("向\r"));	// false
console.log(reg.test("向\n"));	// false
```
...
let reg = /com$/;
console.log(reg.test("www.baidu.com"));	// true
console.log(reg.test("www.abcd.cn"));	// false
// 3. 查询从头到尾是否只有ABCD
let reg = /^ABCD$/i;
console.log(reg.test("aBCd"));	// true
console.log(reg.test("abbbccccd"));	// false
```
...
```javascript
let reg = /[abc]/i;
console.log(reg.test("defg"));	// false
console.log(reg.test("adefg"));	// true
```

上面代码表示，字符串“defg”不包含*a*、*b*、*c*这三个字母中的任一个，而字符串“adefg”包含字母*a*。
...
如果方括号内的第一个字符是 `^`，则表示如果字符串中 **只包含** ^后的字符时，返回false。

[^ tips]: 只要是”^”这个字符是在中括号”[]”中被使用的话就是表示字符类的否。

这里说的是直接在”[]”中使用，不包括嵌套使用，如：/[(^a)]/

该方法出现在“[]”内部，表示“否定”，也就是表示除了这些字符，其它字符都可以匹配。而且这些字符不一定需要连续出现，
...
某些情况下，对于连续序列的字符，通过连字符（`-`）提供简写形式，表示字符的连续范围。比如，`[abc]`可以写成`[a-c]`，`[0123456789]`可以写成`[0-9]`，同理`[A-Z]`表示26个大写字母。
...
- `\b` 匹配词的边界。
  ...
  匹配的只是一个位置，这个位置的一侧是构成单词的字符，另一侧为非单词字符、字符串的开始或结束位置。
...
```javascript
// 1、\s的例子
/\s鲁班七号/.test(" 鲁班七号"); // true
/\s鲁班七号/.test("$鲁班七号"); // false
...
[^ 解析]: 为什么用match，数组长度会是多个

[match](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/String/match)方法的匹配结果为一个数组。这个数组分为4部分，依次为:
